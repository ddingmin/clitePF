	...
	public static Statement T (Statement s, Functions f, TypeMap tm) {
		...
		if (s instanceof Assignment) {
			...
			if (ttype == Type.FLOAT) {
				...
				if (srctype == Type.NEWLINE) {
					src = new Unary(new Operator(Operator.C2S), src);
					ttype = Type.NEWLINE;
				}
			}
		...
		if (s instanceof OutputStatement) {
			...
			Expression test = T (p.expr, f, tm);
			return new OutputStatement(test);
		}
		if (s instanceof OutputStatementCh) {
			OutputStatementCh p = (OutputStatementCh)s;
			Expression test = T (p.expr, f, tm);
			return new OutputStatementCh(test);
		}
		throw new IllegalArgumentException("should never reach here");
	}
}