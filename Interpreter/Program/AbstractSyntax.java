...
class Indenter {               // AST 생성을 위한 display 정의
	public int level;
...
class Declarations extends ArrayList<Declaration> {     // 전역변수 선언
	public void display(int level) {
		Indenter indenter = new Indenter(level);
		indenter.display(getClass().toString().substring(14) + ": ");        // 클래스 보여주기 
...
class Functions extends ArrayList<Function> {           // 함수 AST 생성
	public void display(int level) {
		Indenter indenter = new Indenter(level);
		indenter.display(getClass().toString().substring(14) + ": ");
		for (Function f : this)
			f.display(1);
	}
}

class Function implements Statement {           // 함수 출력 형식
	Type t;
	Variable id;
...
class Call implements Statement, Expression {            // call 형식에 따라서     x = fib();  ||  fib();
	Variable v;
	...
	public void display(int level) {
		...
		indent.display(getClass().toString().substring(14) + ": " + v.id);
		indent.level++;
		if (arguments.size() > 0) {
			indent.display("args =");
...
class Return implements Statement {
	...
	public void display(int level) {
		Indenter indent = new Indenter(level);
		indent.display(getClass().toString().substring(14) + ":");
...
class Type {
	...
	final static Type NEWLINE = new Type("newline");              // newline 추가
	final static Type FLOAT = new Type("float");
	final static Type VOID = new Type("void");                       // void 추가
...
class OutputStatementCh implements Statement {
	Expression expr;
	OutputStatementCh (Expression e) {
		expr = e;
	}
	public void display (int level) {
		Indenter indent = new Indenter(level);
...
abstract class VariableRef implements Expression {      // Variable: 출력
	String id;
...
class Variable extends VariableRef {                // 실제 variable 이름 출력
	Variable(String s) {
		id = s;
	}
	...
	public int hashCode() {                // 같은 주소값인지 중복 체크
		return id.hashCode();
	}
...
abstract class Value implements Expression {
	...
	String newLineValue() {
		assert false : "should never reach here";             // newline
		return " ";
	}
	...
	static Value mkValue(Type type) {
		...
		if (type == Type.NEWLINE)
			return new newLineValue();               // newline
		if (type == Type.FLOAT)
...
class newLineValue extends Value {                 // newlinevalue
	private String value = " ";               // string 갑으로 저장

	newLineValue() {
		type = Type.NEWLINE;
	}

	newLineValue(String v) {
		this();
		value = v;
		undef = false;
	}
...
class Operator {
	...
	final static String INT_REM = "INT%";               // 나머지
	// UnaryOp = !
	...
	final static String FLOAT_REM = "FLOAT%";    // 나머지
	// UnaryOp = !
	...
	final static String C2S = "C2S";           // newline char -> string
	...
	boolean ArithmeticOp() {
		return val.equals(PLUS) || val.equals(MINUS) || val.equals(TIMES) || val.equals(DIV)|| val.equals(REM); 
		// 나머지
	}
	...
	final static String intMap[][] = { { PLUS, INT_PLUS }, { MINUS, INT_MINUS }, { TIMES, INT_TIMES }, { DIV, INT_DIV },{ REM, INT_REM },
	...
	final static String floatMap[][] = { { PLUS, FLOAT_PLUS }, { MINUS, FLOAT_MINUS }, { TIMES, FLOAT_TIMES },
			{ DIV, FLOAT_DIV },{REM,FLOAT_REM}, { EQ, FLOAT_EQ }, { NE, FLOAT_NE }, { LT, FLOAT_LT }, { LE, FLOAT_LE },
	...
	public void display(int level) {
		Indenter indent = new Indenter(level);
		indent.display(getClass().toString().substring(14) + ": " + val);
	}
}
