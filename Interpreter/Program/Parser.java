...
public class Parser {
    ...
    public Program program() {
		Declarations globals = new Declarations();      // 전역변수 선언부 확인
	...
	private void declaration(Declarations d, Functions f) {
		...
		while (!token.type().equals(TokenType.Eof)) {
			if (token.value().equals("main")) {
				f.add(mainFunction(f));         // main이면 main함수
				break;
			}
			
			Variable v = new Variable(token.value());    // main이 아닌 함수이름
			...
			if (token.type().equals(TokenType.LeftParen)) {    // 
				...
				d.add(new VariableDecl(v, t));            // 함수 (타입, 변수명)
	...
	private void function(Functions f, Type t, Variable v) {    // main함수 아닌 함수 인자랑 내용 체크
		cFunction = v;        // v 함수명 
	...
	private Type type () {
		...
		if (token.type().equals(TokenType.Int))
			...
			t = Type.CHAR; 
		else if (token.type().equals(TokenType.Void))          // 함수 타입 void
			t = Type.VOID;
	...
	private Statement statement() {
		...
		else if (token.type().equals(TokenType.Print))  // OutputStatement
			s = outputStatement();
		else if (token.type().equals(TokenType.PrintCh))  // OutputStatementCh
			s = outputStatementCh();
	...
	private OutputStatementCh outputStatementCh() {
		match(TokenType.PrintCh);
		Expression expr = expression();
		match(TokenType.Semicolon);
		return new OutputStatementCh(expr);
	}
	...
	private Call callStatement(Variable v) {          // 함수 call 부분
		match(TokenType.LeftParen);
		...
		return new Call(v, params);     // v 함수명  ,  params 인자값
	}
	
	private Return returnStatement(Variable v) {       // return
		match(TokenType.Return);
		Expression finalReturn = expression();         // 재귀 함수 부분 추가예정
		match(TokenType.Semicolon);
	...
	private Value literal( ) {
		...
		case NewLineLiteral:
			s = match(TokenType.NewLineLiteral);
			return new newLineValue(s);                 // \n 전체 받기
		}
	...
	private boolean isLiteral( ) {
		return token.type().equals(TokenType.IntLiteral) ||
				...
				token.type().equals(TokenType.NewLineLiteral);     // newline
	}
