...
public class StaticTypeCheck {
	...
	public static void V (Declarations d) {             // 변수이름 중복선언
	...
	public static void V(Functions f) {                  // 함수이름 중복 선언
		...
		for (Function fcn : f) {                                                         // 함수 인자랑 전역변수 중복 선언
	...
	public static void V (Program p) {
		...
		if (!mainFound) {
			System.err.println("Error! Cannot find main function!");         // main함수 없을 때 
			System.exit(1);
		}
	...
	public static Type typeOf (Expression e, Functions f, TypeMap tm) {         // 타입체크
	...
	public static void V(Function func, Functions f, TypeMap functionMap) {
		...
		for (int j=0; j < func.body.members.size(); j++) {                             // 함수의 return 오류 체크
			...
			if (s instanceof Return) {
				...
				check(!isReturning, "Return must be the last expressio---------n in function block (in function " + func.id);          //return 뒤에 call있으면 error
				V((Call) s, f, functionMap);
			} else {
				check(!isReturning, "Return must be the last expression -@@@@@@@@@@@@@@@@2in function block (in function " + func.id);  // 
				V(s, f, functionMap);
			}
		}

		if (!func.t.id.equals("void") && !func.id.id.equals("main")) {
			check(isReturning, "Non-void function " + func.id + " missing return statement");       // return이 있으면 error
	...
	public static void V (Statement s, Functions f, TypeMap tm) {
		...
		if (s instanceof Assignment) {
			...
			if (ttype != srctype) {
				if (ttype == Type.FLOAT)
					...
					check( srctype == Type.NEWLINE
							, "mixed mode assigssnment to " + a.target);
				else 
					check( false
							, "mixed mode assignment to " + a.target);
			}
		...
		if (s instanceof OutputStatement) {
			...
			V (p.expr, f, tm);
			check(  typeOf(p.expr, f, tm)== Type.INT||typeOf(p.expr, f, tm)== Type.FLOAT ,
					"print expr error ");
			return;
		}
		if (s instanceof OutputStatementCh) {
			...
			V (p.expr, f, tm);
			check(  typeOf(p.expr, f, tm)== Type.CHAR || typeOf(p.expr, f, tm)== Type.NEWLINE,
					"printCh has non-char expr ");
			return;
		}
		throw new IllegalArgumentException("should never reach here");
	}
