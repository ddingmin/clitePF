...
public class Semantics {
	...
	State M (Statement s, Functions f, State state) {
		...
		if (s instanceof OutputStatement) 
			return M((OutputStatement)s, f, state);
		if (s instanceof OutputStatementCh) 
			return M((OutputStatementCh)s, f, state);
		throw new IllegalArgumentException("should never reach here");
	}
	
	State M (OutputStatement o, Functions f, State state) {
		if (M(o.expr, f, state).type() == Type.BOOL) 
			System.out.print(M(o.expr, f, state).boolValue());
		else if (M(o.expr, f, state).type() == Type.CHAR) 
			System.out.print(M(o.expr, f, state).charValue());
		else if (M(o.expr, f, state).type() == Type.INT) 
			System.out.print(M(o.expr, f, state).intValue());
		else if (M(o.expr, f, state).type() == Type.FLOAT) 
			System.out.print(M(o.expr, f, state).floatValue());
		else
			throw new IllegalArgumentException("No Exist Type in Clite.");
		return state;
	}
	State M (OutputStatementCh o, Functions f, State state) {
		if (M(o.expr, f, state).type() == Type.BOOL) 
			System.out.print(M(o.expr, f, state).boolValue());
		else if (M(o.expr, f, state).type() == Type.CHAR) 
			System.out.print(M(o.expr, f, state).charValue());
			...
			System.out.print(M(o.expr, f, state).floatValue());
		else if (M(o.expr, f, state).type() == Type.NEWLINE) 
			System.out.print(M(o.expr, f, state).newLineValue());
		else
			throw new IllegalArgumentException("No Exist Type in Clite.");
		return state;
	}
	State M(Return r, Functions f, State state) {
		M(r.result, f, state);
		return state;
	}
	...
	//call 부분 인자값들을 스택에 모두 저장 instance of return 만나면 return
		...
		if (s instanceof Block) {
			...
			for (int a = 0; a < b.members.size(); a++) {
				...
				if (block_s instanceof Skip) {
					...
					return M(((Return)block_s).result, f, state);
				} else {
					state = M(block_s, f, state);
				}
			}
		}
	...
	boolean hasReturn(Statement s) {
		if (s instanceof Skip || s instanceof Assignment) {
			...
			return false;
		} else if (s instanceof OutputStatement) {
			OutputStatement p = (OutputStatement)s;
			return false;
		} else if (s instanceof OutputStatementCh) {
			OutputStatementCh p = (OutputStatementCh)s;
			return false;
	...
	Value M (Expression e, Functions f, State state) {
		...
		if (e instanceof Binary) {
			Binary b = (Binary)e;
			return applyBinary (b.op, M(b.term1, f, state), M(b.term2, f, state));
		}
	...
	Value applyUnary (Operator op, Value v) {
		...
		else if (op.val.equals(Operator.C2S))
			return new newLineValue((String)(v.newLineValue( )));
		throw new IllegalArgumentException("should never reach here: "
				+ op.toString());
	}
}